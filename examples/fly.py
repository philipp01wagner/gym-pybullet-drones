...
if __name__ == "__main__":
    ...
    #### Initialize a circular/linear trajectory ######################
    PERIOD = 10
    ...
    if ARGS.trajectory == 1:
        ...
        for i in range(NUM_WP):
            TARGET_POS[i, :] = i*0.00 + INIT_XYZS[0, 0], INIT_XYZS[0, 1], INIT_XYZS[0, 2]
    ...
    print(dir(PYB_CLIENT))
    if ARGS.drone in [DroneModel.HA]:
        p.setGravity(0,0,0)
    else:
        p.setGravity(0,0,-10) 
    ...
    elif ARGS.drone in [DroneModel.HB, DroneModel.HA]:
    ...
    if ARGS.drone in [DroneModel.HA]:
        action = {str(i): np.array([0,0]) for i in range(ARGS.num_drones)}
    START = time.time()
