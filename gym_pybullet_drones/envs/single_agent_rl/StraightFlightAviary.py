...
class StraightFlightAviary(BaseSingleAgentAviary):
    """Single agent RL problem: fligh straight."""
    ...
    def _computeReward(self):
        ...
        #return -((state[0]-1.0)**2 + (state[1]-0.)**2) - 10*state[-1]**2
        square_dist = (state[0]-1.0)**2 + (state[1]-0.)**2
        r = - square_dist
        if square_dist<0.05:
            r+=10
        return r
